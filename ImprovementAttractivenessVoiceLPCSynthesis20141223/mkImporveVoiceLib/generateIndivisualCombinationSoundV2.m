function generateIndivisualCombinationSoundV2(x,fs,dictionary,savefileName,fileSaveDirName)
...
%% Each Prm {f0,vtl}
%%{
soundf0vtlOnlyFileName = [fileSaveDirName '/' 'f0vtl' savefileName '.wav'];
manipulateSoundStructure.f0vtl.outputBuffer = lpcStruct.excitation*0;
...
for ii = 1:length(lpcStructure.temporalPosition)
   inversePreprocessingShape = -2*log(1-lpcStruct.lpcStructure.preEmphasis*cos(lpcStruct.theta(:))) ...
    ...
    filterResponse = minimumPhaseResponse(1:lpcStruct.responseLength);
    response = real(ifft(fft(filterResponse,lpcStruct.fftLSynth).*fft(segment,lpcStruct.fftLSynth)));
    manipulateSoundStructure.f0vtl.outputBuffer(min(length(manipulateSoundStructure.f0.excitation),lpcStruct.olaIndex+round(lpcStruct.lpcStructure.temporalPosition(ii)*Fs))) = ...
        manipulateSoundStructure.f0vtl.outputBuffer(min(length(manipulateSoundStructure.f0.excitation),lpcStruct.olaIndex+round(lpcStruct.lpcStructure.temporalPosition(ii)*Fs)))+response;
end;

audiowrite(soundf0vtlOnlyFileName,manipulateSoundStructure.f0vtl.outputBuffer,8000);
%}

%% f0 vtaf

soundf0vtafOnlyFileName = [fileSaveDirName '/' 'f0vtaf' savefileName '.wav'];

manipulateSoundStructure.f0vtaf.outputBuffer = lpcStruct.excitation*0;
for ii = 1:length(lpcStructure.temporalPosition)
    inversePreprocessingShape = -2*log(1-lpcStruct.lpcStructure.preEmphasis*cos(lpcStruct.theta(:))) ...
        +lpcStruct.lpcStructure.cepstrumList(ii,1)*cos(lpcStruct.theta(:)) ...
        +lpcStruct.lpcStructure.cepstrumList(ii,2)*cos(2*lpcStruct.theta(:));
...
%     fixedLPCspectrum = [fixedHalfLPCspectrum;fixedHalfLPCspectrum(end-1:-1:2)];
    ...
    filterResponse = minimumPhaseResponse(1:lpcStruct.responseLength);
    response = real(ifft(fft(filterResponse,lpcStruct.fftLSynth).*fft(segment,lpcStruct.fftLSynth)));
    manipulateSoundStructure.f0vtaf.outputBuffer(min(length(manipulateSoundStructure.f0.excitation),lpcStruct.olaIndex+round(lpcStruct.lpcStructure.temporalPosition(ii)*Fs))) = ...
        manipulateSoundStructure.f0vtaf.outputBuffer(min(length(manipulateSoundStructure.f0.excitation),lpcStruct.olaIndex+round(lpcStruct.lpcStructure.temporalPosition(ii)*Fs)))+response;
end;

audiowrite(soundf0vtafOnlyFileName,manipulateSoundStructure.f0vtaf.outputBuffer,8000);

%% vtaf vtl

soundvtafvtlOnlyFileName = [fileSaveDirName '/' 'vtafvtl' savefileName '.wav'];

manipulateSoundStructure.vtafvtl.outputBuffer = lpcStruct.excitation*0;
for ii = 1:length(lpcStructure.temporalPosition)
    inversePreprocessingShape = -2*log(1-lpcStruct.lpcStructure.preEmphasis*cos(lpcStruct.theta(:))) ...
        +lpcStruct.lpcStructure.cepstrumList(ii,1)*cos(lpcStruct.theta(:)) ...
        +lpcStruct.lpcStructure.cepstrumList(ii,2)*cos(2*lpcStruct.theta(:));
    ...
    filterResponse = minimumPhaseResponse(1:lpcStruct.responseLength);
    response = real(ifft(fft(filterResponse,lpcStruct.fftLSynth).*fft(segment,lpcStruct.fftLSynth)));
    manipulateSoundStructure.vtafvtl.outputBuffer(min(length(lpcStruct.excitation),lpcStruct.olaIndex+round(lpcStruct.lpcStructure.temporalPosition(ii)*Fs))) = ...
        manipulateSoundStructure.vtafvtl.outputBuffer(min(length(lpcStruct.excitation),lpcStruct.olaIndex+round(lpcStruct.lpcStructure.temporalPosition(ii)*Fs)))+response;
end;

audiowrite(soundvtafvtlOnlyFileName,manipulateSoundStructure.vtafvtl.outputBuffer,8000);
