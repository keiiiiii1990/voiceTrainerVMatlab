function varargout = attractiveImproverDbg003(varargin)
% ATTRACTIVEIMPROVERDBG003 MATLAB code for attractiveImproverDbg003.fig
%      ATTRACTIVEIMPROVERDBG003, by itself, creates a new ATTRACTIVEIMPROVERDBG003 or raises the existing
%      singleton*.
%
%      H = ATTRACTIVEIMPROVERDBG003 returns the handle to a new ATTRACTIVEIMPROVERDBG003 or the handle to
%      the existing singleton*.
%
%      ATTRACTIVEIMPROVERDBG003('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ATTRACTIVEIMPROVERDBG003.M with the given input arguments.
%
%      ATTRACTIVEIMPROVERDBG003('Property','Value',...) creates a new ATTRACTIVEIMPROVERDBG003 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before attractiveImproverDbg003_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to attractiveImproverDbg003_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help attractiveImproverDbg003

% Last Modified by GUIDE v2.5 21-Jan-2015 22:04:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @attractiveImproverDbg003_OpeningFcn, ...
                   'gui_OutputFcn',  @attractiveImproverDbg003_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before attractiveImproverDbg003 is made visible.
function attractiveImproverDbg003_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to attractiveImproverDbg003 (see VARARGIN)

% Choose default command line output for attractiveImproverDbg003
...
% UIWAIT makes attractiveImproverDbg003 wait for user response (see UIRESUME)
...
set(myGUIdata.Play_LPC_Original, 'enable', 'off');
...
function varargout = attractiveImproverDbg003_OutputFcn(hObject, eventdata, handles) 
...
set(myGUIdata.orgVTAFView,'NextPlot','add');

axes(myGUIdata.modVTAFView);
...
crossSection =  [0.2803; ...
    ...
    0.0737];
[X,Y,Z] = cylinder(crossSection,40);
myGUIdata.tract3DOrgOnMod = surf(Z,Y,X);
set(myGUIdata.tract3DOrgOnMod,'FaceAlpha',0.5);

view(-26,12);
axis([0 1 -1 1 -1 1]);
axis off;
axis('vis3d');
myGUIdata.tractRotateOrgAndModVTAF = rotate3d;
set(myGUIdata.tractRotateOrgAndModVTAF,'enable','on');

set(myGUIdata.modVTAFView,'NextPlot','add');
...
axis off;
axis('vis3d');
%set(myGUIdata.tract3DMod,'FaceAlpha',0,'EdgeColor','interp');
...
axes(myGUIdata.humanAspect);
%pic = imread('humanAspect.png');
pic = imread('humanFaceAspect05ForGUI.png');
myGUIdata.hAsp = image(pic);
axis off;
...
if get(myGUIdata.ModVTAFDispBtn,'value') == 1
    ...
    set(myGUIdata.tract3DOrgOnMod,'visible','on');
    set(myGUIdata.tractRotateModVTAF,'enable','on');
    set(myGUIdata.tractRotateOrgAndModVTAF,'enable','on');
elseif get(myGUIdata.ModVTAFDispBtn,'value') == 0
    set(myGUIdata.tract3DMod,'visible','off');
    set(myGUIdata.tract3DOrgOnMod,'visible','off');
    set(myGUIdata.tractRotateModVTAF,'enable','off');
    set(myGUIdata.tractRotateOrgAndModVTAF,'enable','off');
end

%guidata(myGUIdata.AttractivenessImprovementMainGUI,myGUIdata);
end
...
set(myGUIdata.Play_LPC_Original, 'enable', 'on');
...
switch get(myGUIdata.player,'running')
    case 'off'
        ...
        set(myGUIdata.Play_modified_Sound,'string','STOP');
        guidata(handles.AttractivenessImprovementMainGUI,myGUIdata);
        %******************************           Importance part          *******************************%
        set(myGUIdata.player,'userdata',handles.AttractivenessImprovementMainGUI,'TimerFcn', ...
            @sliderCursorModUpdateTimerFunction,'TimerPeriod',myGUIdata.audoPlayTimerPeriod);
        
        playblocking(myGUIdata.player);
        set(myGUIdata.Play_modified_Sound,'string','PLAY');
    case 'on'
        stop(myGUIdata.player);
        set(myGUIdata.Play_modified_Sound,'string','PLAY');
end;
%axes(handles.modVTAFView);
logArea = myGUIdata.crImpSoundStr.newLpcStructure.newLogAreaMatrix(1,:);
cSec = tubeDisplay(logArea(:));
[X,Y,Z] = cylinder(cSec,40);

%set(get(gca,'children'),'xdata',Z,'ydata',Y,'zdata',X,'visible','on');
set(myGUIdata.tract3DMod,'xdata',Z,'ydata',Y,'zdata',X,'visible','on');
%set(myGUIdata.tract3DOrgOnMod,'visible','off');
set(myGUIdata.ModVTAFFrmSlider,'Value',0);
...
logArea = VtafStruct.lpcStructure.logAreaMatrix(1,:);
cSec = tubeDisplay(logArea(:));
[X,Y,Z] = cylinder(cSec,40);
%set(get(gca,'children'),'xdata',Z,'ydata',Y,'zdata',X,'visible','on');
set(myGUIdata.tract3DOrg,'xdata',Z,'ydata',Y,'zdata',X,'visible','on');
set(myGUIdata.OrgVTAFFrmSlider,'Value',0);
...
%sht = floor((length(index30ms)-myGUIdata.VtafStruct.lpcStructure.windowLengthInMs)/floor(fs*myGUIdata.VtafStruct.lpcStructure.frameShiftInMs*0.001));
ii = floor((abs(currentPoint-myGUIdata.VtafStruct.lpcStructure.windowLengthInMs))/floor(fs*0.001));
logArea = VtafStruct.lpcStructure.logAreaMatrix(ii,:);
cSec = tubeDisplay(logArea(:));
[X,Y,Z] = cylinder(cSec,40);
%set(get(gca,'children'),'xdata',Z,'ydata',Y,'zdata',X,'visible','on');
set(myGUIdata.tract3DOrg,'xdata',Z,'ydata',Y,'zdata',X,'visible','on');
set(myGUIdata.OrgVTAFFrmSlider,'Value',ii);
%{
...
function sliderCursorModUpdateTimerFunction(obj, event, string_arg)
%obj is "audioplayer"Object , which has "userData" property.
...
%axes(handles.orgVTAFView);
%axes(myGUIdata.modVTAFView);
%sht = floor((length(index30ms)-myGUIdata.crImpSoundStr.newLpcStructure.lpcStructure.windowLengthInMs)/floor(fs*myGUIdata.crImpSoundStr.newLpcStructure.lpcStructure.frameShiftInMs*0.001));
ii = floor((abs(currentPoint-myGUIdata.crImpSoundStr.newLpcStructure.lpcStructure.windowLengthInMs))/floor(fs*0.001));

logAreaO = VtafStruct.lpcStructure.logAreaMatrix(ii,:);
cSecO = tubeDisplay(logAreaO(:));
[XO,YO,ZO] = cylinder(cSecO,40);
%set(get(gca,'children'),'xdata',Z,'ydata',Y,'zdata',X,'visible','on');
set(myGUIdata.tract3DOrgOnMod,'xdata',ZO,'ydata',YO,'zdata',XO,'visible','on','FaceAlpha',0,'EdgeColor','black');

logAreaM = myGUIdata.crImpSoundStr.newLpcStructure.newLogAreaMatrix(ii,:);
cSecM = tubeDisplay(logAreaM(:));
[XM,YM,ZM] = cylinder(cSecM,40);
...
%set(get(gca,'children'),'xdata',Z,'ydata',Y,'zdata',X,'visible','on');
%set(myGUIdata.tract3DMod,'xdata',ZM,'ydata',YM,'zdata',XM,'visible','on','FaceAlpha',1,'Facecolor','magenta');
%set(myGUIdata.tract3DMod,'xdata',ZM,'ydata',YM,'zdata',XM,'visible','on','FaceAlpha',1,'Facecolor','magenta','MeshStyle','row');
set(myGUIdata.tract3DMod,'xdata',ZM,'ydata',YM,'zdata',XM,'visible','on','FaceAlpha',1,'MeshStyle','row');
%colormap(hot)
set(myGUIdata.ModVTAFFrmSlider,'Value',ii);
...
if get(hObject,'Min') < get(hObject,'Value')&&  get(hObject,'Value') < get(hObject,'Max')
    if isequal(get(myGUIdata.tract3DOrg,'visible'),'on') && isequal(get(myGUIdata.tract3DMod,'visible'),'on')
        VTAFFrmIndex = floor(get(hObject,'Value'));
        %org vtaf processing
        ologArea = VtafStruct.lpcStructure.logAreaMatrix(VTAFFrmIndex,:);
        ocSec = tubeDisplay(ologArea(:));
        [oX,oY,oZ] = cylinder(ocSec,40);
        set(myGUIdata.tract3DOrg,'xdata',oZ,'ydata',oY,'zdata',oX,'visible','on');
        %drawnow        
        %mod vtaf processing
        ...
        set(myGUIdata.tract3DMod,'xdata',mZ,'ydata',mY,'zdata',mX,'visible','on');
        drawnow
...
%%For debug and LPC version this GUI function
% --- Executes on button press in load_wavfileForTestBtn.
function load_wavfileForTestBtn_Callback(hObject, eventdata, handles)
% hObject    handle to load_wavfileForTestBtn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
myGUIdata = guidata(handles.AttractivenessImprovementMainGUI);
...
% --- Executes on button press in Play_LPC_Original.
function Play_LPC_Original_Callback(hObject, eventdata, handles)
% hObject    handle to Play_LPC_Original (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
myGUIdata = guidata(handles.AttractivenessImprovementMainGUI);
...
x = modSoundStr.lpcStruct.outputBuffer;
fs = modSoundStr.lpcStruct.Fs;
minus4 = max(1,round((length(x)/fs-4)*fs));
%sound(x(minus4:end)/max(abs(x(minus4:end)))*0.99,fs);
myGUIdata.minus4 = minus4;
...
switch get(myGUIdata.player,'running')
    case 'off'
        myGUIdata.player = audioplayer(x(minus4:end)/max(abs(x(minus4:end)))*0.9,fs);
        set(myGUIdata.Play_LPC_Original,'string','STOP');
        guidata(handles.AttractivenessImprovementMainGUI,myGUIdata);
        %******************************           Importance part          *******************************%
        set(myGUIdata.player,'userdata',handles.AttractivenessImprovementMainGUI,'TimerFcn', ...
            @sliderCursorModUpdateTimerFunction,'TimerPeriod',myGUIdata.audoPlayTimerPeriod);
        ...
        playblocking(myGUIdata.player);
        set(myGUIdata.Play_LPC_Original,'string','PLAY');
    case 'on'
        stop(myGUIdata.player);
        set(myGUIdata.Play_LPC_Original,'string','PLAY');
end;
